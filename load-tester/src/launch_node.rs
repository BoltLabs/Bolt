...
pub fn launch_node(
    ...
    let data_dir = tmp_dir.join(format!("bolt_load_test_{}", i));
    match fs::create_dir(&data_dir) {
        Ok(_) => {}
        Err(ref e) if e.kind() == io::ErrorKind::AlreadyExists => {
            let _ = fs::remove_file(data_dir.join("data.ldb"));
        }
        r => r.chain_err(|| "failed to create bolt_node data directory")?,
    }
    ...
    let config = json!({
        ...
        "node": {
            ...
            "preconfigured_representatives": [
                "blt_1ysz9ssfuxdbwix49fky77axfxontx6f7nzy9pwnim398aerg79pigupakgt"
            ],
            "inactive_supply": "0",
    ...
    let child = Command::new(bolt_node)
        .arg("--data_path")
        .arg(&data_dir)
        .arg("--daemon")
        .spawn_async(&handle)
        .chain_err(|| "failed to spawn bolt_node")?;
...
pub fn connect_node<C: Connect>(
    ...
    })).then(|x| x.chain_err(|| "failed to call bolt_node RPC"))
            .map(|_| ()),
    ) as _
}
