...
#include <bolt/node/xorshift.hpp>
...
opencl (opencl_a)
{
	...
	auto count (rai::rai_network == rai::rai_networks::bolt_test_network ? 1 : std::min (max_threads_a, std::max (1u, std::thread::hardware_concurrency ())));
