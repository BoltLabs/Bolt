...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % BOLT_VERSION_MAJOR % BOLT_VERSION_MINOR).c_str ());
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (BOLT): ") + wallet.format_balance (balance.first));
			if (!balance.second.is_zero ())
			{
				final_text += "\nPending: " + wallet.format_balance (balance.second);
			}
...
blt (new QRadioButton ("BAN")),
mblt (new QRadioButton ("mBLT")),
ublt (new QRadioButton ("uBLT")),
back (new QPushButton ("Back")),
...
wallet (wallet_a)
{
	ratio_group->addButton (blt);
	ratio_group->addButton (mblt);
	ratio_group->addButton (ublt);
	ratio_group->setId (blt, 0);
	ratio_group->setId (mblt, 1);
	ratio_group->setId (ublt, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (blt);
	scale_layout->addWidget (mblt);
	scale_layout->addWidget (ublt);
