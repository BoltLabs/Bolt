#include <bolt/node/openclwork.hpp>
...
__kernel void bolt_work (__global ulong * attempt, __global ulong * result_a, __global uchar * item_a)
{
	...
	if (result >= 0xfffffe0000000000ul) 
	{
		*result_a = attempt_l;
	}
}
							...
							if (!error_a)
							{
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										kernel = clCreateKernel (program, "bolt_work", &kernel_error);
