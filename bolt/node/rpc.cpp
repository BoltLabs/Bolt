...
#include <bolt/node/rpc.hpp>

#include <bolt/lib/interface.h>
#include <bolt/node/node.hpp>

#include <ed25519-donna/ed25519.h>

#ifdef BOLT_SECURE_RPC
#include <bolt/node/rpc_secure.hpp>
#endif
...
void rai::rpc_handler::blt_from_raw ()
{
...
void rai::rpc_handler::blt_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::mblt_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::mblt_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::ublt_from_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::ublt_to_raw ()
{
	std::string amount_text (request.get<std::string> ("amount"));
	rai::uint128_union amount;
	if (!amount.decode_dec (amount_text))
...
void rai::rpc_handler::version ()
{
	...
	response_l.put ("node_vendor", boost::str (boost::format ("Bolt %1%.%2%") % BOLT_VERSION_MAJOR % BOLT_VERSION_MINOR));
	response (response_l);
}
...
void rai::rpc_handler::process_request ()
{
	try
	{
		...
		else if (action == "mblt_from_raw")
		{
			mblt_from_raw ();
		}
		else if (action == "mblt_to_raw")
		{
			mblt_to_raw ();
		}
		...
		else if (action == "blt_from_raw")
		{
			blt_from_raw ();
		}
		else if (action == "blt_to_raw")
		{
			blt_to_raw ();
		}
		else if (action == "password_change")
		...
		else if (action == "ublt_from_raw")
		{
			ublt_from_raw ();
		}
		else if (action == "ublt_to_raw")
		{
			ublt_to_raw ();
		}
